...
impl Repl {
    ...
    pub fn new(opts: Rc<dyn CompilerOpts>, runner: Rc<dyn TRunProgram>) -> Repl {
        ...
        let start_program_fe = frontend(opts.clone(), &[starter_empty_program]).unwrap().compileform().clone();
        ...
        parse_sexp(self.loc.clone(), input_taken.bytes())
            ...
            .and_then(|parsed_program| {
                ...
                if is_helper {
                    ...
                    let program = frontend(self.opts.clone(), &[built_program])?.compileform().clone();
                    self.evaluator
